...
1. This window shows the region the program is looking at.
...
6. How much time is left until program presses the "drink all potions" button.
...
it means that only `1/50`the of the (2) motion preview should turn white in order for the program to
understand that it's time to catch a fish.
10. The program has functionality to periodically press the `B` button in order for your Terraria character to drink
all potions. This is useful for fishing and luck potions, but can be disabled by this checkbox.
11. Change the interval in seconds between automatically drinking potions. AFAIK, fisher potions last for 3 minutes, so I've
...
deleting any preset or switching to other preset. The preset system is poorly tested, so I recommend backing up
...
4. Configure coordinates so that the fishing rod's line, float and water surface are visible in preview window.
...
7. **Remove the line from the water before starting AutoFishing.**
8. Save preset. (not necessary)
9. Click "start fishing". 
10. Cast the line into the water, but make sure that your mouse cursor doesn't get into the scanned region.
11. Observe how AutoFisher behaves. Tweak settings if it doesn't work like expected.


### Recommendations
1. Equip an anti-knockback shield just in case
2. Build a fishing shelter so no one disturbs you
3. Build a wall so that the background doesn't interfere with the program
4. Place a couple of torches so that they are not visible in preview area but also so that the room has enough
light to be independent from the day-night cycle
